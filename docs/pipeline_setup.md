...
```
./fly sp -t <YOUR_TARGET_NAME> configure -c pipeline.yml -p addcar --load-vars-from=credentials.yml
...
```
./fly -t <YOUR_TARGET_NAME> unpause-pipeline -p addcar
...
```
<YOUR_CONCOURSE_URL>/pipelines/addcar
```
...
```
./fly -t <YOUR_TARGET_NAME> destroy-pipeline -p addcar
```